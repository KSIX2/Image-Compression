...
class Region:
    def __init__(self, img, coords):
        self.img_width, self.img_height = img.crop(coords).size
        self.coords = coords

        hist = img.crop(coords).histogram()
        self.detail = get_detail(hist)
        self.avg_color = calculate_avgColor(img.crop(coords))
    ...
    def branch(self):
        ...
        if(self.detail > THRESHOLD and width - left >= 5 and height - top >= 5):
